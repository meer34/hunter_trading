...
public interface StockOutRepo extends JpaRepository<StockOut, Long>, JpaSpecificationExecutor<StockOut>{
	...
	@Query("FROM StockOut so WHERE EXISTS (SELECT 1 FROM StockOutProduct sop, Product p WHERE sop.product = p.id AND sop.stockOut = so.id AND p.name = :name AND p.size = :pSize AND p.colour = :colour AND p.brand = :brand  AND p.productType = :productType )")
	Page<StockOut> findAllStockOutsByProductDetails(Pageable pageable, String name, String pSize, String colour, String brand, String productType);
