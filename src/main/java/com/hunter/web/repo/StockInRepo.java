...
public interface StockInRepo extends JpaRepository<StockIn, Long>, JpaSpecificationExecutor<StockIn>{
	...
	@Query("FROM StockIn si WHERE EXISTS (SELECT 1 FROM Product p WHERE p.stockIn = si.id AND p.name = :name AND p.size = :pSize AND p.colour = :colour AND p.brand = :brand AND p.productType = :productType)")
	Page<StockIn> findAllStockInsByProductDetails(Pageable pageable, String name, String pSize, String colour, String brand, String productType);
